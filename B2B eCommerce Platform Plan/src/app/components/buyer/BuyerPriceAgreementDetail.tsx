// ============================================================
// Chi tiết Thoả thuận giá / HĐ khung — Buyer
// ============================================================
...
import {
  ...
  FileText, Building2, BarChart3, Calendar, TrendingDown,
} from 'lucide-react';
...
export function BuyerPriceAgreementDetail() {
  const { id } = useParams();
  ...
  if (!agreement) {
    return (
      <div className="container mx-auto px-4 py-6 text-center">
        ...
        <Button className="mt-4" onClick={() => navigate('/price-agreements')}>
  ...
  const usedPercent = agreement.totalContractValue > 0
    ? Math.round((agreement.usedValue / agreement.totalContractValue) * 100)
    : 0;
  const remaining = agreement.totalContractValue - agreement.usedValue;
  const totalSaved = agreement.items.reduce((s, i) => s + (i.originalPrice - i.agreedPrice) * i.minQuantity, 0);

  return (
    <div className="container mx-auto px-4 py-6 space-y-6">
      <AppBreadcrumb items={[
        { label: 'Trang chủ', href: '/' },
        { label: 'Thoả thuận giá', href: '/price-agreements' },
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={() => navigate('/price-agreements')}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <SummaryCard label="Tiết kiệm ước tính" value={formatPrice(totalSaved)} icon={TrendingDown} />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <InfoRow label="Mã thoả thuận" value={agreement.agreementNumber} />
            <InfoRow label="Loại" value={agreement.type} />
            <InfoRow label="Tiêu đề" value={agreement.title} />
      ...
      {/* Sản phẩm trong thoả thuận */}
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Sản phẩm thoả thuận ({agreement.items.length})</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <FileText className="h-4 w-4" /> Đơn hàng sử dụng thoả thuận ({orders.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          {orders.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-6">Chưa có đơn hàng nào sử dụng thoả thuận này</p>
          ) : (
            <div className="space-y-2">
              {orders.map(order => (
                <div key={order.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/20 border hover:bg-muted/40 transition-colors cursor-pointer"
